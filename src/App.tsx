...
function LazyPage({ children }: { children: React.ReactNode }) {
  return (
    <Suspense
      fallback={
        <div
          className="min-h-screen bg-[#0a1628] flex flex-col items-center justify-center gap-4"
          role="status"
          aria-live="polite"
        >
          <div
            className="w-10 h-10 border-2 border-[#5a6b4e]/20 border-t-[#8a9d79] rounded-full animate-spin"
            aria-hidden="true"
          />
          <span className="text-white/70 text-sm font-light">Chargement…</span>
        </div>
      }
    >
      {children}
...
export default function App() {
  ...
  const splashVisible = showSplash && !reduceMotion;
  ...
  return (
    <>
      {splashVisible && <SplashScreen onFinish={() => setShowSplash(false)} />}
      <div
        aria-hidden={splashVisible || undefined}
        inert={splashVisible || undefined}
      >
