...
export default function Dashboard() {
  ...
  const { data: orderStats, refetch: refetchOrderStats } =
    trpc.admin.orderStats.useQuery(undefined, {
      enabled: isAuthenticated && user?.role === "admin",
    });
  const { data: customerStats, refetch: refetchCustomerStats } =
    trpc.admin.customerStats.useQuery(undefined, {
      enabled: isAuthenticated && user?.role === "admin",
    });
  const { data: ordersList, refetch: refetchOrders } =
    trpc.admin.orderList.useQuery(undefined, {
      enabled: isAuthenticated && user?.role === "admin",
    });
  const { data: customersList, refetch: refetchCustomers } =
    trpc.admin.customerList.useQuery(undefined, {
      enabled: isAuthenticated && user?.role === "admin",
    });
  ...
  const updateOrderStatus = trpc.admin.orderUpdateStatus.useMutation({
    ...
    onError: e =>
      toast(e.message || "Échec de la mise à jour de la commande", "error"),
  });
  const deleteOrder = trpc.admin.orderDelete.useMutation({
    ...
    onError: e =>
      toast(e.message || "Échec de la suppression de la commande", "error"),
  });
  const updateCustomerStatus = trpc.admin.customerUpdateStatus.useMutation({
    ...
    onError: e =>
      toast(e.message || "Échec de la mise à jour du client", "error"),
  });
  const deleteCustomer = trpc.admin.customerDelete.useMutation({
    ...
    onError: e =>
      toast(e.message || "Échec de la suppression du client", "error"),
  });
  ...
  return (
    <div className="min-h-screen bg-[#0a1628] pt-20">
      <SEO
        ...
        description="Tableau de bord d'administration Ciné Kin Premium"
        noIndex
      />
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
              activeTab === "orders"
                ...
                : "bg-white/[0.03] text-white/60 hover:text-white/65"
            }`}
          >
            Commandes
          </button>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    <th className="text-left text-white/60 text-xs font-medium px-4 py-3">
                      ID
                    </th>
                    <th className="text-left text-white/60 text-xs font-medium px-4 py-3">
                      Client
                ...
                <tbody>
                  {ordersList && ordersList.length > 0 ? (
                    ordersList.map(order => (
                      ...
                      >
                        <td className="text-white/60 text-sm px-4 py-3">
                          #{order.id}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          ...
                          <div className="text-white/55 text-xs capitalize">
                            {order.planType}
                          </div>
                        </td>
                    ...
                    <tr>
                      <td
                        colSpan={7}
                        className="text-center text-white/55 text-sm py-12"
                      >
                        Aucune commande
                      </td>
                    </tr>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            aria-label={`Supprimer le client ${customer.name || customer.id}`}
                            onClick={() => {
                              if (confirm("Supprimer ?"))
