// Behavioral proof, not just type-checking: mirrors useAuth.test.tsx's
// harness for the reseller portal (its own dedicated ck_reseller_sid
// session, `reseller.me` / `reseller.logout`).
...
function TestHarness() {
  const { reseller, isAuthenticated, isLoading, logout, logoutError } =
    useReseller();
  ...
  return (
    <div>
      <div>Tableau de bord ({reseller?.name})</div>
...
describe("useReseller logout — behavioral proof (real cache, not mocked)", () => {
  afterEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });
